...
class VcfDedupRunner(object):
    ...
    SUPPORTED_VARIANT_CALLERS = [
        constants.STRELKA_VARIANT_CALLER,
        constants.STARLING_VARIANT_CALLER,
        constants.DUPLICATION_FINDER,
        constants.PLATYPUS_VARIANT_CALLER,
        constants.GENERIC_VARIANT_CALLER,
    ]
    SUPPORTED_EQUALITY_MODES = [
        constants.INCLUDE_ALTERNATE,
        constants.EXCLUDE_ALTERNATE,
    ]
    SUPPORTED_SELECTION_METHODS = [
        constants.SELECTION_METHOD_AF,
        constants.SELECTION_METHOD_ALLELE_CALLS,
    ...
    def __init__(self, config):
        ...
        if log_file != "" and log_file is not None:
            logging.basicConfig(
                level=log_level, filename=log_file, format=self.LOGGING_FORMAT
            )
        else:
        ...
        if config["temp_folder"] is not None:
            ...
            self.temp_folder = os.path.dirname(
                os.path.realpath(
                    self.output_vcf
    ...
    def sanity_checks(self):
        ...
        try:
            ...
            if sort_threads < 1:
                message = (
                    "'sort_threads' must be positive and greater than zero [%s]"
                    % str(sort_threads)
                )
                logging.error(message)
                raise VcfDedupInputError(message)
        ...
        if self.config["variant_caller"] not in self.SUPPORTED_VARIANT_CALLERS:
            message = (
                "Non supported variant caller [%s]. The list of supported variant callers is %s"
                % (
                    self.config["variant_caller"],
                    ", ".join(self.SUPPORTED_VARIANT_CALLERS),
                )
            )
            logging.error(message)
            raise VcfDedupInputError(message)
        if self.config["selection_method"] not in self.SUPPORTED_SELECTION_METHODS:
            message = (
                "Non supported selection method [%s]. The list of supported selection methods is %s"
                % (
                    self.config["selection_method"],
                    ", ".join(self.SUPPORTED_SELECTION_METHODS),
                )
            )
            logging.error(message)
            raise VcfDedupInputError(message)
        if self.config["equality_mode"] not in self.SUPPORTED_EQUALITY_MODES:
            message = (
                ...
                % (
                    self.config["variant_caller"],
                    ", ".join(self.SUPPORTED_EQUALITY_MODES),
                )
            )
            logging.error(message)
            raise VcfDedupInputError(message)
        if (
            self.config["variant_caller"] == constants.STRELKA_VARIANT_CALLER
            and self.config["selection_method"]
            == constants.SELECTION_METHOD_ALLELE_CALLS
        ):
            message = (
                "Non supported selection method [%s] for variant caller [%s] which has no genotypes"
                % (
                    constants.SELECTION_METHOD_ALLELE_CALLS,
                    constants.STRELKA_VARIANT_CALLER,
                )
        ...
        has_valid_sample_idx = (
            "sample_idx" in self.config and self.config["sample_idx"] is not None
        )
        if has_valid_sample_idx:
            try:
                sample_idx = int(self.config["sample_idx"])
            except ValueError:
                message = "'sample_idx' must be numeric and found [%s]" % str(
                    sample_idx
                )
                logging.error(message)
                raise VcfDedupInputError(message)
        has_valid_sample_name = (
            "sample_name" in self.config and self.config["sample_name"] is not None
        )
        if (
            self.config["variant_caller"]
            in [constants.STRELKA_VARIANT_CALLER, constants.STARLING_VARIANT_CALLER]
            and not has_valid_sample_idx
            and not has_valid_sample_name
        ):
            message = (
                "Variant caller [%s] requires that either [sample_name] or [sample_idx] are provided"
                % self.config["variant_caller"]
            )
            logging.error(message)
            raise VcfDedupInputError(message)
        if "sort_mem_percentage" in self.config:
            try:
                ...
                if sort_mem_percentage <= 0 or sort_mem_percentage > 100:
                    message = (
                        "'sort_mem_percentage' must be in the range [1, 100] and it was found [%s]"
    ...
    def __sort(self):
        ...
        self.sorted_vcf = os.path.join(
            self.temp_folder, "." + input_basename + ".sorted.vcf.gz"
        )
    ...
    def run(self):
        ...
        if self.equality_mode == constants.INCLUDE_ALTERNATE:
            comparer = VariantComparerWithAlternate()
        elif self.equality_mode == constants.EXCLUDE_ALTERNATE:
            comparer = VariantComparerNoAlternate()
        # selects the appropriate transformer
        if self.variant_caller == constants.STRELKA_VARIANT_CALLER:
        ...
        elif self.variant_caller == constants.STARLING_VARIANT_CALLER:
        ...
        elif self.variant_caller == constants.PLATYPUS_VARIANT_CALLER:
            transformer = PlatypusVcfDedupper(
                self.sorted_vcf, self.output_vcf, comparer, self.selection_method
            )
        elif self.variant_caller == constants.GENERIC_VARIANT_CALLER:
            transformer = GenericVcfDedupper(
                self.sorted_vcf, self.output_vcf, comparer, self.selection_method
            )
        elif self.variant_caller == constants.DUPLICATION_FINDER:
            transformer = DuplicationFinder(self.sorted_vcf, self.output_vcf, comparer)
