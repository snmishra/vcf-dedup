...
class AbstractVcfTransformer(object, metaclass=ABCMeta):
    ...
    def process_vcf(self):
        ...
        for variant in self.reader:
            if len(variant.ALT) > 1:
                logging.error(
                    "The VCF contains multi-allelic variants %s:%s%s>%s. Please split them before processing!"
                    % (
                        variant.CHROM,
                        str(variant.POS),
                        str(variant.REF),
                ...
                raise VcfFormatError(
                    "The VCF contains multi-allelic variants. Please split them before processing!"
                )
        ...
        after = time.time()
        logging.info(
            "Finished processing %s variants in %s seconds"
            % (str(count), str(after - before))
        )
    ...
    def _variant2dict(self, writer, variant):
        ...
        samples = [
            writer._format_sample(variant.FORMAT, sample) for sample in variant.samples
        ]
        return ffs + samples
...
class DuplicationFinder(AbstractVcfTransformer):
    ...
    def _transform_variant(self, variant):
        ...
        # next block
        if prev_variant is not None and not self.variant_comparer.equals(
...
class AbstractVcfDedupper(AbstractVcfTransformer):
    def __init__(
        ...
        if selection_method == constants.SELECTION_METHOD_AF:
            self._select_variants = self._select_mode_af
        elif selection_method == constants.SELECTION_METHOD_QUALITY:
            self._select_variants = self._select_mode_quality
        elif selection_method == constants.SELECTION_METHOD_ALLELE_CALLS:
            self._select_variants = self._select_mode_allele_calls
        elif selection_method == constants.SELECTION_METHOD_ARBITRARY:
    ...
    def _select_mode_af(self, variants):
        """
        Returns the variants with the highest allele frequency. If there is a
        collision returns the variants with the maximum quality. Otherwise
        returns an arbitrary variant.
        :param variants:
        :return:
        """
        allele_frequencies = {}
    ...
    def _merge_variants(self, variants):
        ...
        """
        logging.info(
            "Merging duplicate variants at %s:%s:%s>%s"
            % (
                variants[0].CHROM,
                int(variants[0].POS),
                variants[0].REF,
    ...
    def _get_sample(self, variant):
        ...
        if self.sample_name is not None and self.sample_name != "":
            samples = [
                ...
                if sample.sample == self.sample_name
            ]
            if len(samples) == 0:
                raise VcfFormatError(
                    "Sample '%s' is not available in the VCF" % self.sample_name
                )
            sample = samples[0]
        else:
    ...
    def _write_last_block(self):
        ...
        self.variants = []
        logging.info(
            "Found %s positions having duplicated variants"
            % (str(self.duplications_count))
        )
        self._flush_buffer(self.writer)
    ...
    def _calculate_allele_calls(self, variant):
        ...
        """
        logging.debug(
            "Calculating ratio of alternate genotypes for %s:%s"
...
class StrelkaVcfDedupper(AbstractVcfDedupper):
    ...
    def _calculate_AF(self, variant):
        ...
        """
        logging.debug(
            "Calculating somatic ratio of supporting reads for %s:%s"
            % (variant.CHROM, str(variant.POS))
        )
        af = 0
        ...
        if variant.is_snp:
            ...
            if reference + "U" in format and alternate + "U" in format:
                ...
                af = (
                    alternate_ac / (alternate_ac + reference_ac)
                    if alternate_ac + reference_ac > 0
                    else 0
                )
            ...
            if "TIR" in format and "TAR" in format:
                ...
                af = (
                    float(indel_ac) / (alternate_ac + indel_ac)
                    if alternate_ac + indel_ac > 0
                    else 0
                )
        elif variant.is_sv:
...
class StarlingVcfDedupper(AbstractVcfDedupper):
    ...
    def _calculate_AF(self, variant):
        ...
        """
        logging.debug(
            "Calculating ratio of supporting reads for %s:%s"
            % (variant.CHROM, str(variant.POS))
        )
        af = 0
        format = self._get_sample(variant).data._asdict()
        if "AD" in format and len(format["AD"]) == 2:
            ...
            af = (
                float(alternate_ac) / (alternate_ac + reference_ac)
                if alternate_ac + reference_ac > 0
                else 0
            )
        return af
...
class GenericVcfDedupper(AbstractVcfDedupper):
    """
    This generic VCF dedupper is aimed to support any VCF based on standard
    annotations in the VCF specification.
    """
