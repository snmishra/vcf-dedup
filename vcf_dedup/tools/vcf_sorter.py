...
class VcfSorter(object):
    ...
    def __init__(
        ...
        logging.info(
            "Memory percentage to be used by sort [%s]" % str(self.mem_percentage)
        )
    ...
    def __sort(self):
        ...
        sort_command = (
            ...
            "--buffer-size=%s%%" % str(self.mem_percentage)
            if self.mem_percentage is not None
            else "",
            self.output_vcf,
        )
        logging.info("Running [%s]" % sort_command)
        sort = subprocess.Popen(
            sort_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        sort.communicate()
